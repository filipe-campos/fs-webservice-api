...
@app.route('/dashboard/api/v1.0/account/delete_all', methods=['DELETE'])
...
def ebitda_period():
    return Response(ebitdaRoute.get_ebitda_period(request), mimetype='application/json')
...
## -- UPLOAD -- ##
@app.route('/dashboard/api/v1.0/upload/launchs', methods=['POST'])
def upload_launchs():
    return Response(uploadRoute.upload_launchs(request), mimetype='application/json')

@app.route('/dashboard/api/v1.0/upload/account', methods=['POST'])
def upload_coa():
    return Response(uploadRoute.upload_account(request), mimetype='application/json')

@app.route('/dashboard/api/v1.0/upload/balance', methods=['POST'])
def upload_balance():
    return Response(uploadRoute.upload_balance(request), mimetype='application/json')
...
if __name__ == '__main__':
    app.run(host='dashboard.fiscoserv.com.br', port=5000, ssl_context=('fullchain.pem', 'privkey.pem'), debug=False)
